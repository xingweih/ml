...
def generate_weight(name, shape, stddev, wd=5e-4):
	var = generate_variables(name, shape,
...
def inference_vgg(images, weight_bias=None, dropoutRate=1.0):
	...
	with tf.variable_scope('conv5') as scope:
		...
		preActivation = tf.nn.bias_add(conv, biases)
		conv5 = tf.nn.relu(preActivation, name=scope.name)
	...
	with tf.variable_scope('conv6') as scope:
		...
		preActivation = tf.nn.bias_add(conv, biases)
		conv6 = tf.nn.relu(preActivation, name=scope.name)
	...
	with tf.variable_scope('conv7') as scope:
		...
		preActivation = tf.nn.bias_add(conv, biases)
		conv7 = tf.nn.relu(preActivation, name=scope.name)
	...
	with tf.variable_scope('conv8') as scope:
		...
		preActivation = tf.nn.bias_add(conv, biases)
		conv8 = tf.nn.relu(preActivation, name=scope.name)
	...
	with tf.variable_scope('conv9') as scope:
		...
		preActivation = tf.nn.bias_add(conv, biases)
		conv9 = tf.nn.relu(preActivation, name=scope.name)
	...
	with tf.variable_scope('conv10') as scope:
		...
		preActivation = tf.nn.bias_add(conv, biases)
		conv10 = tf.nn.relu(preActivation, name=scope.name)
	...
	with tf.variable_scope('conv11') as scope:
		...
		preActivation = tf.nn.bias_add(conv, biases)
		conv11 = tf.nn.relu(preActivation, name=scope.name)
	...
	with tf.variable_scope('conv12') as scope:
		...
		preActivation = tf.nn.bias_add(conv, biases)
		conv12 = tf.nn.relu(preActivation, name=scope.name)
	...
	with tf.variable_scope('conv13') as scope:
		...
		preActivation = tf.nn.bias_add(conv, biases)
		conv13 = tf.nn.relu(preActivation, name=scope.name)
...
def loss(logits, labels):
	...
	'''
	crossEntropy = tf.nn.sigmoid_cross_entropy_with_logits(
						labels=labels, logits=logits, name='cross_entropy_per_example')
...
def predict(logits):
	...
	return tf.cast(resCompare, tf.int32)
...
def accuracy(logits, labels):
	...
	ones = tf.ones([batch], tf.int32)
	mean = tf.equal(mean, ones)
	return tf.cast(mean, tf.int32)
	#return tf.reduce_mean(isEqual)
	
def add_loss_summaries(totalLoss):
	print('----------------train.add_loss_summaries---------------------')
	lossAverages = tf.train.ExponentialMovingAverage(0.9, name='avg')
...
def train(totalLoss, globalStep):
	print('----------------train.train---------------------')
	numBatchPerEpoch = NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN / batch
	decaySteps = int(numBatchPerEpoch * NUM_EPOCHS_PER_DECAY)
	...
	lossAveragesOp = add_loss_summaries(totalLoss)

	with tf.control_dependencies([lossAveragesOp]):
		opt = tf.train.GradientDescentOptimizer(lr)
		grads = opt.compute_gradients(totalLoss)
	
	applyGradientOp = opt.apply_gradients(grads, globalStep)
	...
	variableAverages = tf.train.ExponentialMovingAverage(
		MOVING_AVERAGE_DECAY, globalStep)
	variablesAveragesOp = variableAverages.apply(tf.trainable_variables())

	with tf.control_dependencies([applyGradientOp, variablesAveragesOp]):
		trainOp = tf.no_op(name='train')
	return totalLoss, trainOp, lr
